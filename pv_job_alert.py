...
Pipeline:
  ...
  3. LinkedIn via Apify REST -> same cooldown + score gate
  4. Combine -> single email
...
def filter_swiss_by_cooldown(swiss_jobs, cooldowns):
    ...
    2026-08-16: also used for the LinkedIn/Apify source, so that path inherits
    the identical cooldown check and score_job gate. No new scoring policy.
    """
    kept = []
    for j in swiss_jobs:
